...
class TelegramNotifier:
    ...
    async def send(self, message: str) -> bool:
        ...
        Returns:
            bool: 是否发送成功
        """
        if not message.startswith("[P6]"):
            message = f"[P6] {message}"
...
class WebSocketScalper:
                    ...
                    if success:
                        ...
                        if balance > 0:
                            ...
                            if stats['per_10k'] > SWITCH_COST_PER_10K:
                                logger.warning(f"📊 磨损过高: ¥{stats['per_10k']:.2f}/万 > ¥{SWITCH_COST_PER_10K}/万，切换账号...")
    ...
    async def switch_account(self, reason: str):
        ...
        if TG_NOTIFY_FEE_PAUSE:
            await self.tg_notifier.send(
                f"🔄 切换账号\n"
                f"当前: {self.current_account_name}\n"
                f"原因: {reason}\n"
                f"统计: {self.cycle_count}循环 | ${stats['pnl']:+.2f} | ¥{stats['per_10k']:.2f}/万"
            )
